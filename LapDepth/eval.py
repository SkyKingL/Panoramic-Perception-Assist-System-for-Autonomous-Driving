...
parser.add_argument('--data_path', type=str, default="./example/kitti_demo.jpg")
...
def main():
    ...
    if args.evaluate is True:
        ...
        for i in range(test_len):
            ...
            test_model.load_state_dict(torch.load(models_list[i], map_location='cpu'))
            # test_model.load_state_dict(torch.load(models_list[i]))  原来是cuda:0
            test_model.eval()
            if args.dataset == 'KITTI':
        ...
        test_model = Model
        test_model.load_state_dict(torch.load(models_list[0], map_location='cpu'))
        # test_model.load_state_dict(torch.load(models_list[0]))  原来是cuda:0
        test_model.eval()
        print("=> No validation")
    ...
    if args.img_save is True:
        ...
        for idx, (rgb_data, gt_data, gt_dense, filename) in enumerate(val_loader):
            ...
            gt_data = Variable(gt_data.to(device))
            input_img = Variable(rgb_data.to(device))
            gt_data = gt_data.clamp(0, args.max_depth)
            if args.use_dense_depth is True:
                gt_dense = Variable(gt_dense.to(device))
            ...
            folder_name_list = ['/output_depth', '/output_depth_cmap_gray', '/output_depth_cmap_jet', '/ground_truth',
                                '/input_rgb', '/dense_gt', '/dense_gt_cmap_gray', '/dense_gt_cmap_jet']
            img_name_list = ['/' + filename, '/cmap_gray_' + filename, '/cmap_jet_' + filename, '/gt_' + filename,
                             '/rgb_' + filename, '/gt_dense_' + filename, '/gt_dense_cmap_gray_' + filename,
