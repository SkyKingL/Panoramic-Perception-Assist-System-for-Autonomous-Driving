...
class WSConv2d(nn.Conv2d):
    ...
    def forward(self, x):  # 权重参数标准化 创新点之一
        weight = self.weight
        weight_mean = weight.mean(dim=1, keepdim=True).mean(dim=2, keepdim=True).mean(dim=3, keepdim=True)
        weight = weight - weight_mean  # 去均值
        std = weight.view(weight.size(0), -1).std(dim=1).view(-1, 1, 1, 1) + 1e-5  # 标准差
...
# pre-activation based upsampling conv block  基于预激活的上采样卷积模块
...
# pre-activation based conv block  基于预激活的卷积模块
...
class deepFeatureExtractor_ResNext101(nn.Module):  # ResNext101
    def __init__(self, args, lv6=False):
        ...
        else:  # lv6为False则删除encoder中的layer4和fc层
    ...
    def forward(self, x):
        out_featList = []
        feature = x
        for k, v in self.encoder._modules.items():  # _modules.items()是网络结构的各层
            if k == 'avgpool':  # 如果是avgpool层 退出循环
                break
            feature = v(feature)  # 将feature一层一层传递
            # feature = v(features[-1])
            # features.append(feature)
            # 如果网络结构中包含relu, layer1, layer2, layer3, [layer4],则将经过这层的feature添加到out_featList中
            if any(x in k for x in self.layerList):
                out_featList.append(feature)
        return out_featList  # 返回的是一个list
...
class deepFeatureExtractor_VGG19(nn.Module):  # VGG19
    def __init__(self, args, lv6=False):
...
class deepFeatureExtractor_DenseNet161(nn.Module):  # DenseNet161
    def __init__(self, args, lv6=False):
...
class deepFeatureExtractor_InceptionV3(nn.Module):  # InceptionV3
    def __init__(self, args, lv6=False):
...
class deepFeatureExtractor_MobileNetV2(nn.Module):  # MobileNetV2 速度比较快
...
class deepFeatureExtractor_ResNet101(nn.Module):  # ResNet101
    def __init__(self, args, lv6=False):
...
class deepFeatureExtractor_EfficientNet(nn.Module):  # EfficientNet
...
# ASPP Module ↓------------------------------------------------------------------------↓
# 三个ASPP模块的区别???
class Dilated_bottleNeck(nn.Module):  # reduction和ASPP写在了同一个类中
...
class Lap_decoder_lv5(nn.Module):
    def __init__(self, args, dimList):
        ...
        self.ASPP = Dilated_bottleNeck(norm, act, dimList[3])  # -----ASPP-----
    ...
    def forward(self, x, rgb):  # x是特征图
        cat1, cat2, cat3, dense_feat = x[0], x[1], x[2], x[3]  # 越来越小的特征图：1/2 1/4 1/8 1/16
        # rgb_lv 差异特征
        ...
        lap_lv5_up = self.upscale(lap_lv5, scale_factor=2, mode='bilinear')  # 上采样2倍的R5
        ...
        lap_lv4_img = lap_lv4 + lap_lv5_up  # D4
        lap_lv3_img = lap_lv3 + self.upscale(lap_lv4_img, scale_factor=2, mode='bilinear')  # D3
        lap_lv2_img = lap_lv2 + self.upscale(lap_lv3_img, scale_factor=2, mode='bilinear')  # D2
        final_depth = lap_lv1 + self.upscale(lap_lv2_img, scale_factor=2, mode='bilinear')  # D1
        final_depth = torch.sigmoid(final_depth)
        return [(lap_lv5) * self.max_depth, (lap_lv4) * self.max_depth, (lap_lv3) * self.max_depth,  # return的是深度(数字)数组
...
class LDRN(nn.Module):
    ...
    def forward(self, x):
        out_featList = self.encoder(x)  # 返回的是一个list(多个特征图)
        # interpolate函数既可以做上采样也可以做下采样(这里是下采样)，看传入的倍率(scale_factor)
        # 这里下采样得到的特征图并不是经过encoder的???
        ...
        lap1 = x - rgb_up  # 计算差异特征，通过差异放大边界信息
        ...
        print(depth.shape)
        return d_res_list, depth  # d_res_list 是D5~D2的深度，depth 是最终深度
